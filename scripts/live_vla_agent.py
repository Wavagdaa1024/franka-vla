...
class DualRealSenseStreamer:
    """Streams dual RealSense cameras in dedicated background threads at 30fps."""
    def __init__(self, front_serial, wrist_serial):
    ...
    def _worker(self, serial, role):
        ...
        try:
            ...
            print(f"[Camera {role}] Online and streaming at 30 fps (S/N: {serial}).")
...
def main():
    ...
    print("=" * 75)
    print(f"  LIVE VLA GPU INFERENCE AGENT ({args.profile.upper()} MODE)")
    print("=" * 75)
    ...
    cams = DualRealSenseStreamer(FRONT_SERIAL, WRIST_SERIAL)
    cams.start()
    ...
    if args.checkpoint:
        ...
        if ckpt_target.exists():
            print(f"[Model] Loading fine-tuned weights from {ckpt_target.name}...")
