...
# -*- coding: utf-8 -*-
"""
Pure Synchronous VLA Inference Agent for Franka Panda.
Runs on Windows Server (Strict GPU 1 Isolation: RTX 5090).

Workflow:
  1. Franka Controller completes 15 steps and enters stationary state (zero blur).
  2. Receives current robot joint angles and gripper state over TCP.
  3. Grabs fresh, sharp frames from Dual RealSense (Front & Wrist).
  4. Runs single-pass Pi0.5 inference (~430ms on RTX 5090) to generate 15-step chunk.
  5. Enforces 5-DOF Rigid Vertical Orientation Lock (Roll/Pitch 0.00° tilt).
  6. Enforces Hard Table Floor Limit (Z >= +7.0mm).
  7. Sends 15-step action chunk back to Franka Controller.
"""
...
# Default single-task red cube checkpoint
DEFAULT_SINGLE_TASK_CKPT = PROJECT_ROOT / "outputs" / "checkpoints" / "pi05_jointpos_expert" / "action_expert_final.pt"
# Multi-task LoRA checkpoint
...
class DualRealSenseStreamer:
    ...
    def get_frames(self, max_age_s: float = 0.5, max_skew_s: float = 0.15):
        with self.lock:
            now = time.time()
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Synchronous VLA Inference Agent for Franka Panda")
    ...
    parser.add_argument("--live", action="store_true", help="Explicitly allow live policy streaming")
    args = parser.parse_args()

    if not args.live:
        parser.error("BLOCKED: live VLA requires explicit --live flag after controller safety verification")

    print("=" * 80)
    print("  SYNCHRONOUS VLA INFERENCE AGENT (STRICT GPU 1 ISOLATION: RTX 5090)")
    print("=" * 80)
    ...
    print(f"[*] Nullspace Stabilizer:   {'DISABLED' if args.disable_nullspace else f'ACTIVE (Strict 0.00° Vertical Downward Lock)'}")
    print(f"[*] Invert Joint 0:         {args.flip_lr}")

    # 1. Start Dual RealSense Streams
    print("\n[1/3] Initializing Dual RealSense Streams...")
    cams = DualRealSenseStreamer(FRONT_SERIAL, WRIST_SERIAL, fps=args.fps)
    cams.start()

    # 2. Load Model onto GPU 1 (RTX 5090)
    ...
    # Determine checkpoint: explicit -> single-task red cube -> multitask LoRA
    if args.checkpoint:
        ckpt_path = Path(args.checkpoint)
    elif DEFAULT_SINGLE_TASK_CKPT.exists():
        ckpt_path = DEFAULT_SINGLE_TASK_CKPT
    elif DEFAULT_LORA_CKPT.exists():
        ckpt_path = DEFAULT_LORA_CKPT
    else:
        ckpt_path = None
    ...
    if ckpt_path and ckpt_path.exists():
        ...
        if is_lora:
            ...
            print(f"[Model OK] LoRA Adapters loaded! (Step: {ckpt.get('step', '?')}, Loss: {ckpt.get('loss', 0.0):.4f})")
        else:
            ...
            print(f"[Model OK] Single-Task Red Cube Expert{vision_tag} loaded! (File: {ckpt_path.name}, Step: {ckpt.get('step', '?')}, Loss: {ckpt.get('loss', 0.0):.4f})")
    else:
        print("[Model Warning] No custom checkpoint loaded. Running base pre-trained weights.")

    # 3. Connection & Synchronous Inference Loop
    while True:
        ...
        while True:
            try:
                ...
                print(f"[Network] Connected to Franka Controller! Awaiting state requests...")
                break
            except Exception as e:
                print(f"  [Waiting for Franka] Please ensure sync_franka_server.py is running on {args.host} ({e})")
                time.sleep(2.0)

        loop_cnt = 0
        try:
            while True:
                # Receive robot state from stationary arm
                msg = recv_json(sock)
                if msg is None:
                ...
                if img_f is None or img_w is None:
                    print(f"  [Sync #{loop_cnt:03d} Warning] RealSense frames missing! Sending hold chunk.")
                    resp = {
                        "type": "action_chunk",
                        "loop": loop_cnt,
                ...
                if args.profile == "jointpos":
                    ...
                    # Enforce 5-DOF Rigid Vertical Lock & Hard Floor Limit
                    ...
                    print(f"  [Sync Cycle #{loop_cnt:03d}] Infer: {t_infer:5.1f}ms | Tilt Max: {max_tilt_deg:4.2f}° | Target Δp: [{dp[0]:+.3f}, {dp[1]:+.3f}, {dp[2]:+.3f}]m | Grip: {g_valid[0]:.2f}{clamp_str}")
                    ...
                    print(f"  [Sync Cycle #{loop_cnt:03d}] Infer: {t_infer:5.1f}ms | Mean dq: [{mean_v[0]:+.3f}, {mean_v[1]:+.3f}, {mean_v[2]:+.3f}, {mean_v[3]:+.3f}, {mean_v[4]:+.3f}, {mean_v[5]:+.3f}, {mean_v[6]:+.3f}] | Grip: {g_valid[0]:.2f}")
