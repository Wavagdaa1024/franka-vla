...
1. **[01. 离线环境与安全自检 (01_offline_sanity_checks.md)](01_offline_sanity_checks.md)**
   ...
   - 验证可微正向运动学 (DFK) 梯度求导 (`tests\test_differentiable_fk.py`)
   - 运行双目相机硬件抽样体检与持续实时流 (`scripts\check_cameras.bat` / `scripts\live_camera.bat`)
   - 执行 GPU 1 单卡隔离影子推理自检 (`tests\shadow_run_pi05.py`) 与 Agent 离线自测 (`scripts\run_agent.bat --mock`)
...
3. **[03. 实机闭环 VLA 部署四步法 (03_live_vla_deployment.md)](03_live_vla_deployment.md)**
   ...
   - Windows GPU 端启动推理主入口 (`scripts\run_agent.bat`)
   - 核心推理模式切换：`--raw` (DFK 端到端)、`--rtc` (闭环自稳滤波)、`--mock` (离线自检)
   - 物理急停手持与回车安全闭环

4. **[04. 模型训练与微调 (04_model_training.md)](04_model_training.md)**
   - GPU 0 物理单卡独立训练，不影响 GPU 1 推理
   - 推荐首选：带 DFK 笛卡尔空间损失与垂直向下倾角约束的 LoRA (`scripts\train_cartesian_lora_gpu0.bat`)
   - 纯关节空间 LoRA 微调入口 (`scripts\train_red_cube_lora_gpu0.bat`)
